...
def rellenar_registro(archivo_entrada, config_path="config.yaml"):
    ...
    # 3️⃣ Rellenar horas y firmas en la tabla principal (solo si hay número de día)
    for table in doc.tables:
        for row in table.rows:
            primera_celda = row.cells[0].text.strip()
            # Saltar encabezados, filas de texto y filas vacías (sin número de día)
            if not primera_celda.isdigit():
                continue
    ...
    # 4️⃣ Añadir firma en bloque "Recibido por el trabajador"
    for i, p in enumerate(doc.paragraphs):
        if "Recibido por el trabajador" in p.text:
            # Insertar un párrafo nuevo debajo
            nuevo_parrafo = doc.paragraphs[i+1] if i+1 < len(doc.paragraphs) else doc.add_paragraph()
